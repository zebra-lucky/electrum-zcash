...
from electrum_zcash.util import UserCancelled, InvalidPassword, WalletFileException, get_new_wallet_name
...
from electrum_zcash.plugin import run_hook, Plugins
...
WIF_HELP_TEXT = (_('WIF keys are typed in Zcash Electrum, based on script type.') + '\n\n' +
                 _('A few examples') + ':\n' +
...
MSG_PASSPHRASE_WARN_ISSUE4566 = _("Warning") + ": "\
                              ...
                              + _("Due to a bug, old versions of Zcash Electrum will NOT be creating the "
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def run_upgrades(self, storage: WalletStorage, db: 'WalletDB') -> None:
        ...
        if db.requires_split():
            self.hide()
            msg = _("The wallet '{}' contains multiple accounts, which are no longer supported since Zcash Electrum 2.7.\n\n"
    ...
    def init_network(self, network: 'Network'):
        message = _("Zcash Electrum communicates with remote servers to get "
                  "information about your transactions and addresses. The "
                  "servers all fulfill the same purpose only differing in "
                  "hardware. In most cases you simply want to let Zcash Electrum "
