...
from electrum_zcash.storage import WalletStorage, StorageReadWriteError
from electrum_zcash.wallet_db import WalletDB
from electrum_zcash.wallet import Wallet, InternalAddressCorruption, Abstract_Wallet
from electrum_zcash.wallet import update_password_for_directory
...
from electrum_zcash.invoices import PR_PAID, PR_FAILED
from electrum_zcash import blockchain
from electrum_zcash.network import Network, TxBroadcastError, BestEffortRequestFailed
from electrum_zcash.interface import PREFERRED_NETWORK_PROTOCOL, ServerAddr
...
from .uix.dialogs.zcash_kivy import TorWarnDialog
...
from electrum_zcash.util import (NoDynamicFeeEstimates, NotEnoughFunds,
...
class ElectrumWindow(App, Logger):
    ...
    def on_new_intent(self, intent):
        ...
        if scheme in [ZCASH_BIP21_URI_SCHEME, PAY_BIP21_URI_SCHEME]:
            self.set_URI(data)
    ...
    def scan_qr(self, on_complete):
        ...
        PythonActivity = autoclass('org.kivy.android.PythonActivity')
        SimpleScannerActivity = autoclass("org.zcash.electrum.qr.SimpleScannerActivity")
        Intent = autoclass('android.content.Intent')
    ...
    def init_ui(self):
        ...
        if self.testnet:
            self.icon = os.path.dirname(KIVY_GUI_PATH) + "/icons/electrum-zcash-testnet.png"
        else:
            self.icon = os.path.dirname(KIVY_GUI_PATH) + "/icons/electrum-zcash.png"
    ...
    def get_max_amount(self):
        from electrum_zcash.transaction import PartialTxOutput
    ...
    def notify(self, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../../' + self.icon)
            notification.notify('Zcash Electrum', message,
                            app_icon=icon, app_name='Zcash Electrum')
        except ImportError:
